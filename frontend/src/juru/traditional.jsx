...
const TraditionalPage = () => {
   ...
   useEffect(() => {
      const fetchProducts = async () => {
         try {
            const response = await axios.get('http://localhost:8081/api/products?category=traditional')
            setProducts(response.data.filter((product) => product.category === 'traditional'))
