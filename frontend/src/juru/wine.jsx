...
const WinePage = () => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8081/api/products?category=wine"
        );
        setProducts(
          response.data.filter((product) => product.category === "wine")
        );
        console.log("Wine products:", response.data); // 디버깅 로그 추가
      } catch (error) {
  ...
  return (
    <div className="product-page">
      <img
        src={require("../images/wine/main.jpg")} // 경로 수정
        alt="Main Product"
