...
public class AdultVerificationController {
	...
	public ResponseEntity<?> verifyAdult(@RequestBody Map<String, String> requestData) {
	    try {
	        String apiUrl = "https://api.tilko.net/api/v1.0/certification/adult";

	        HttpHeaders headers = new HttpHeaders();
	        headers.setContentType(MediaType.APPLICATION_JSON);
	        headers.set("API-Key", "YOUR_API_KEY_HERE"); // Tilko API-Key

	        Map<String, String> body = new HashMap<>();
	        body.put("Name", requestData.get("Name"));
	        body.put("RegistrationNumber", requestData.get("RegistrationNumber")); // 암호화된 주민등록번호
